  ...
  return (
    <div className="relative w-full h-full bg-[#02130f]"> {/* Dark Midori/Evergreen base */}
      <Canvas shadows dpr={[1, 1.5]} gl={{ antialias: false }}>
        ...
        <fog attach="fog" args={['#02130f', 5, 15]} />

        <Suspense fallback={null}>
          <Environment preset="forest" />
          <ambientLight intensity={0.2} />
          
          {/* Main Key Light - Tealish/Midori */}
          <spotLight
            position={[10, 10, 10]}
            angle={0.15}
            penumbra={1}
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-wrap justify-center gap-4 z-20">
        {animations.map((anim) => (
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-xs font-bold tracking-[0.2em] uppercase transition-all duration-500 ${
              animation === anim.val
